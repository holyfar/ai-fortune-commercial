...
分析要求：
1. 首先根据出生年、月、日、时推断出天干地支、八字五行
2. 分析日主强弱、喜用神
3. 从事业财运、婚姻感情、健康运势三个方面进行分析
4. 给出具体的发展建议和注意事项
5. 语气要专业、温和、有说服力
...
分析要求：
1. 结合星座特质和当前星象进行解读
2. 运势分为：整体运势、爱情运、事业运、财运、健康运
3. 每个运势维度给出1-5星的评价
4. 给出幸运数字、幸运颜色、贵人星座等实用信息
5. 给出当日建议
...
请给出详细的命理分析。`,
  ...
  async queryStarFortune(input: StarInput): Promise<string> {
    ...
    const userMessage: ChatMessage = {
      role: 'user',
      content: `请为${starNames[input.star] || input.star}分析${timeNames[input.type]}运势：`,
    }
